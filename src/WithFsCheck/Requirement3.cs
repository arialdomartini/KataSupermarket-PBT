...
internal record Promo(Product Product, int Threshold, int Discount);
...
public class Requirement3
{
    ...
    [Property]
    Property promotions_let_the_user_save_money()
    {
        var useCases =
            from promotion in Arb.Generate<Promo>()
            from products in Arb.Generate<List<Product>>()
            select new {Promotion=promotion, Products=products.Append(promotion.Product)};
